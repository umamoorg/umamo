# Umamo's copy of the RPM spec template of the JDK 21 jpackage that packages the Linux installers, changed in two
# places.  The preun scriptlet removes the menu entry and the .uma registration only when the package is really
# being removed (its first argument is 0), not when an upgrade replaces it: an RPM upgrade runs the old package's
# preun after the new package's post, so the stock template undoes every upgrade's desktop integration
# (JDK-8301856, fixed in JDK 22).  And both scriptlets register best-effort, as the DEB's postinst and prerm do: on
# a system with no desktop menu directories xdg-desktop-menu fails, and a failed preun would make the package
# impossible to remove.  jpackage fills this file in like its own template, so it names no placeholder outside the
# lines that use one.  Re-diff it against the packaging JDK's template whenever that JDK changes.
