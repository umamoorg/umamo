 ...
 * which a large export can run out of.  So the shell warns a jar launch with a small limit, and an export
 * that runs out tells a jar user the command that raises it.  Android has no such choice and passes none.
 *
...
/** The limit below which a jar launch is warned: under it, a 3 × 8192² model's export can run out. */
internal const val LOW_HEAP_NOTICE_BELOW_BYTES = 3L * 1024 * 1024 * 1024
 ...
 */
internal fun lowHeapNoticeDue(hostHeap: HostHeap?, noticeEnabled: Boolean): Boolean = hostHeap != null && !hostHeap.packagedLaunch && noticeEnabled && hostHeap.maxBytes < LOW_HEAP_NOTICE_BELOW_BYTES
 ...
 * raises it; the installed launcher already uses half of the machine's memory, so there is no option to
 * offer it.
 *
...
internal fun exportOutOfMemoryAlert(destinationName: String, hostHeap: HostHeap?): AlertRequest =
	if (hostHeap != null && !hostHeap.packagedLaunch) {
