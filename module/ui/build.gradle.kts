...
kotlin {
	...
	// NOTE: the source-set visibility above is enforced by the compiler; `java.*` is NOT — every target
	// here is JVM-based, so commonMain CAN resolve it. Keeping JVM-only code out of commonMain is a
	// convention in THIS module, preserving the option of a non-JVM target (Kotlin/Native, for iPadOS);
	// the root build's `checkCommonSourcePurity` task is what holds the line. :format has gone further
	// and carries an iosArm64 target, which turns the same rule into a compiler guarantee there — the
	// pattern to copy if this module ever needs the same certainty.
