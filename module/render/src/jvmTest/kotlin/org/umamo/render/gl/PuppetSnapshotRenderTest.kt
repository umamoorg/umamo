...
class PuppetSnapshotRenderTest {
	...
	/**
	 * The one-quad model: world x and z in [20, 80].  Mesh positions are the model's own y-down space,
	 * which the deform flips to world z-up, so the quad's mesh y runs from -80 to -20.
	 *
	 * @return PuppetModel The model.
	 */
	private fun quadModel(): PuppetModel {
		val positions = floatArrayOf(20f, -20f, 80f, -20f, 20f, -80f, 80f, -80f)
		val quad =
			Drawable(
				id = quadId,
				name = quadId.raw,
		...
		return PuppetModel(
			...
			rootChildren = listOf(OrgChild.Drawable(quad.id)),
			rootPartId = null,
		)
	}
	...
	fun transparentCaptureLeavesTheCanvasClearAndDrawsNoTintOrAxes() {
		...
		val image = renderer.renderSnapshot(ViewportCamera(0f, 0f, 1f), imageSize, imageSize, FrameBackdrop.Transparent)
	...
	fun solidCaptureFillsTheCanvasAndBlendsTheQuadOverIt() {
		...
		val renderer = viewportRenderer(GlRenderDevice())
		val image = renderer.renderSnapshot(ViewportCamera(0f, 0f, 1f), imageSize, imageSize, FrameBackdrop.Clear(1f, 0.5f, 0f, 1f))
	...
	fun aTiledCaptureMatchesAWholeOne() {
		...
		val whole = renderer.renderSnapshot(camera, 211, 157, FrameBackdrop.Grid)
		val tiled = renderer.renderSnapshot(camera, 211, 157, FrameBackdrop.Grid, tileEdge = 64)
		var worst = 0
		var worstAt = 0
	...
	fun aTiledCaptureOfACorpusRigMatchesAWholeOne() {
		...
		val whole = renderer.renderSnapshot(camera, width, height, FrameBackdrop.Transparent)
		val tiled = renderer.renderSnapshot(camera, width, height, FrameBackdrop.Transparent, tileEdge = 128)
