...
/**
 * Inserts a key at [position] on [target]'s channel, holding whatever the channel already evaluates to
 * there, as one undo step.
 *
 * The sheet's "insert key here" - distinct from `I`, which captures the CURRENT pose and the current
 * value.  Holding the evaluated value means the insert is shape-preserving: the curve through the new key
 * is the curve that was already there, so the key is a handle to edit from rather than an edit in itself.
 *
 * @param KeyableTarget target The entity and channel.
 * @param Parameter parameter The parameter whose axis to insert on.
 * @param Float position The new key's parameter value.
 */
fun EditorSession.insertChannelKeyAt(target: KeyableTarget, parameter: Parameter, position: Float) {
	mutate(KeyformChange.InsertKey(target.channel)) { model ->
		val posed = pose.value + (parameter.id to position)
		val heldValue = model.channelValueAt(target, posed) ?: return@mutate model
		model.withChannelKeyCaptured(target, parameter, posed, heldValue)
	}
}

/**
 * Removes the key at [position] from [target]'s channel on [parameter], as one undo step.
 *
 * Positional rather than pose-based, because the sheet knows exactly which mark was clicked and must not
 * re-derive it from wherever the playhead happens to be sitting.
 *
 * @param KeyableTarget target The entity and channel.
 * @param Parameter parameter The parameter whose axis to remove from.
 * @param Float position The key's parameter value.
 */
fun EditorSession.removeChannelKeyAt(target: KeyableTarget, parameter: Parameter, position: Float) {
	mutate(KeyformChange.RemoveKey(target.channel)) { model ->
		model.withChannelKeyRemoved(target, parameter, pose.value + (parameter.id to position))
	}
}
