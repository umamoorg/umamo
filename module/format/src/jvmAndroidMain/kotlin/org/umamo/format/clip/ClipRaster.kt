...
internal object ClipRaster {
	...
	private val BEGIN_TAG = "BlockDataBeginChunk".toByteArray(Charsets.UTF_16BE)
	private val END_TAG = "BlockDataEndChunk".toByteArray(Charsets.UTF_16BE)
	private val STATUS_TAG = "BlockStatus".toByteArray(Charsets.UTF_16BE)
	private val CHECKSUM_TAG = "BlockCheckSum".toByteArray(Charsets.UTF_16BE)
	...
	private fun inflateTile(tileBytes: ByteArray, destination: ByteArray): Int {
		...
		inflater.setInput(tileBytes, 4, tileBytes.size - 4)
		var total = 0
		try {
			while (!inflater.finished() && total < destination.size) {
				val produced = inflater.inflate(destination, total, destination.size - total)
				if (produced == 0) {
	...
	private fun parseAttributeGrid(attribute: ByteArray): AttributeGrid? {
		...
		val parameterTag = "Parameter".toByteArray(Charsets.UTF_16BE)
