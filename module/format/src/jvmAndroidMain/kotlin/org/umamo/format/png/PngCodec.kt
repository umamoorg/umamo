...
/**
 * Pure-Kotlin PNG codec (desktop JVM and Android), read and write.
 *
 ...
 * thumbnail pipeline needs.  Uses only java.util.zip for DEFLATE, so it decodes byte-identically
 * on both targets (no javax.imageio / BitmapFactory, which is exactly what this replaces).
 */
public object PngCodec : RasterCodec {
	...
	override fun read(bytes: ByteArray): RasterImage {
		...
		val raw = inflateZlib(idatBytes, 0, idatBytes.size)
	...
	override fun write(model: RasterImage): ByteArray {
		val out = ByteArrayOutputStream(model.rgba.size / 2 + 128)
		...
		writeChunk(out, "IDAT", deflateZlib(filtered))
