	...
	override fun write(model: RasterImage): ByteArray {
		...
		for (fileRow in 0 until model.height) {
			...
			for (column in 0 until model.width) {
				out.write(model.rgba[source + 2].toInt() and 0xFF) // B
				out.write(model.rgba[source + 1].toInt() and 0xFF) // G
				out.write(model.rgba[source].toInt() and 0xFF) // R
				out.write(model.rgba[source + 3].toInt() and 0xFF) // A
				source += 4
			}
		}
