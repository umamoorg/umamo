...
internal object PsdLayerRecords {
	...
	fun parse(bytes: ByteArray): PsdParse {
		// ByteBuffer is big-endian by default, which is exactly PSD's byte order - no manual swaps.
		val buffer = ByteBuffer.wrap(bytes)

		// PSD: File Header - signature '8BPS', version, reserved(6), channels, height, width, depth, mode.
		val signature = buffer.string(4)
		...
		val colorMode = buffer.short.toInt() and 0xFFFF
		...
		val colorModeLength = buffer.int
		val colorModeData = ByteArray(colorModeLength)
		buffer.get(colorModeData)

		skipLengthPrefixed(buffer) // PSD: Image Resources section (u32 length + data) - not needed here

		buffer.int // PSD: Layer and Mask Information section length (unused here)
		buffer.int // PSD: Layer Info length (unused here)
		...
		val layerCount = abs(buffer.short.toInt())
		...
		repeat(layerCount) {
			...
			repeat(channelCount) {
				val channelId = buffer.short.toInt() // signed: 0/1/2… color, -1 alpha, -2/-3 masks
				val channelLength = buffer.int.toLong() and 0xFFFFFFFFL // u32 byte length of the data block
				channelInfos += PsdChannelInfo(id = channelId, length = channelLength)
			}

			val blendSignature = buffer.string(4) // PSD: must be '8BIM'
			require(blendSignature == "8BIM") { "bad layer blend signature '$blendSignature'" }
			val blendKey = buffer.string(4) // PSD: 4-char blend mode key, e.g. 'norm'
			val opacityByte = buffer.get().toInt() and 0xFF // PSD: opacity 0..255
			val clipping = buffer.get().toInt() and 0xFF // PSD: clipping, 0=base, 1=non-base (clipped)
			// PSD: layer-record flags @ +0x0B; bit 1 (0x02) set means the layer is hidden (eye off).
			val flags = buffer.get().toInt() and 0xFF
			buffer.get() // PSD: filler (unused)
			...
			val nameLength = buffer.get().toInt() and 0xFF // PSD: legacy Pascal-string name length
			val name = buffer.string(nameLength)
	 ...
	 *
	 * @param ByteBuffer buffer      The whole-file buffer.
	 ...
	 */
	private fun findAdditionalInfoInt(buffer: ByteBuffer, regionStart: Int, regionEnd: Int, key: String): Int? {
		...
		while (scanPosition + 16 <= regionEnd) {
			...
			if ((signature == "8BIM" || signature == "8B64") && buffer.asciiAt(scanPosition + 4, 4) == key) {
				return buffer.getInt(scanPosition + 12) // the u32 right after the block's length field
			}
			scanPosition++
		}
		return null
	...
	/**
	 * Reads [count] ASCII bytes at an absolute buffer index without moving the cursor.
	 *
	 * @param Int index The absolute byte offset to read from.
	 * @param Int count Number of bytes to decode.
	 * @return String the decoded text.
	 */
	private fun ByteBuffer.asciiAt(index: Int, count: Int): String {
		val chars = CharArray(count)
		for (charOffset in 0 until count) {
			chars[charOffset] = (get(index + charOffset).toInt() and 0xFF).toChar()
		}
		return String(chars)
	}
	 ...
	 *
	 * @param ByteBuffer buffer Positioned at the 4-byte length field.
	 */
	private fun skipLengthPrefixed(buffer: ByteBuffer) {
	...
	/**
	 * Reads [count] bytes and decodes them as ASCII (PSD signatures and legacy layer names).
	 *
	 * @param Int count Number of bytes to read.
	 * @return String the decoded text.
	 */
	private fun ByteBuffer.string(count: Int): String {
		val raw = ByteArray(count)
		get(raw)
