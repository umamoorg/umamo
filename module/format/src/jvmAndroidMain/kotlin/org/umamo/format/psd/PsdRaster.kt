...
internal object PsdRaster {
	 ...
	 *
	 * PSD ZIP channels are standard zlib streams (header + Adler-32), so a default [Inflater] decodes
	 * them.  A short or malformed stream yields a partially filled (zero-padded) buffer rather than
	 * throwing, mirroring [org.umamo.format.clip.ClipRaster].
	 *
	...
	private fun inflate(bytes: ByteArray, offset: Int, length: Int, expected: Int): ByteArray {
		...
		try {
			while (!inflater.finished() && total < out.size) {
				val produced = inflater.inflate(out, total, out.size - total)
				if (produced == 0) {
			...
			// Leave the rest zero-filled; the layer is best-effort rather than a hard failure.
		} finally {
			inflater.end()
		}
		return out
