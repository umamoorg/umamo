...
/**
 * Sample-driven code generator for the Cubism model classes. Reads each sample's main.xml,
 ...
 * (attribute-bearing) tags for hand-writing. Emits the model classes; DescriptorGenerator then
 * derives the serialization descriptors + registration from them, so run it afterwards.
 *
...
class ModelGenerator {
	...
	fun generate() {
		...
		println("=== now run DescriptorGenerator (-Dcmo3.generateDescriptors=true) to refresh the descriptors ===")
