...
- **Sources** — the source-art linking table: artwork file → layer (the inventory as of the last read) → tile → drawables, each with a status (present / missing file, bound / bound by name / unbound layer, unplaced tile) shown as the row icon's traffic-light tint (`signalGood` / `signalCaution` / `signalBad`, the word in its tooltip) and a filter; a layer row dragged onto a tile row (or the reverse) rebinds it, a tile row's chip picks a layer or unbinds, and Add Artwork… (`file.addArtwork`) brings another file into the OPEN document as one undo step, packed into the gaps around the existing art and adjustable on the operation strip; the header's Reload (`document.reloadArtwork`) re-reads every present file and lands the changed layers as one step, and a relink (`sources.relink`) pulls the layer's art at once when its file is present.  A layer the file lost keeps its row (named and sized as last read) with a review chip carrying the matcher's proposal (accept / relink by hand / leave); the header's Match Automatically (`sources.matchAutomatically`) rebinds every confident proposal as one step with the threshold on the strip, a reload or a replace does the same at its own Match Threshold row before it mints anything, and a proposal naming the fresh drawable a reload minted retires that drawable when taken; a file row's chip or context menu offers Replace Artwork… (`sources.replaceArtwork`) and Reload This File.  An unbound layer row's chip or context menu ignores the layer (`sources.ignoreLayer`: a reload never mints it, the row muted) or stops ignoring it, and Delete Art marks the layer ignored too under the `import.deleteArtIgnoresLayer` setting; a lost row's tooltip says whether the layer left the file or a replacement lacks its key; a reload follows a layer's eye toggle into a drawable the rigger never toggled.
...
**Current status: Phase F is complete - the binding is persisted and editable, the reconcile by key (F1), the watcher (F2), and the fuzzy matcher with its review states (F3) are all built.**  The identity is ONE representation: the reader's `LayerId.raw` is the key and the reader says how strong it is (`SourceLayer.idIsStable`); it persists on the tile as `AtlasTile.source: SourceLayerRef(sourceId, layerKey, stableKey)`, and the document lists its artwork files with their pixel-free layer inventories as `PuppetModel.sources: List<ArtSource>` (the baseline a reconcile diffs against).  The Sources space shows and edits the table (relink, unbind, Add Artwork, Reload).  `:reimport`'s `KeyReconciler` classifies bindings by key and `ArtworkReloadPlanner` turns the matched and added halves into an `ArtworkReload` the session applies (`withArtworkReloaded`) and the UI packs and commits (`ReloadArtworkFlow.kt`): a changed layer's tile is superseded by a NEW tile (`<root>~<n>`, `AtlasTile.replaces`; pixels are immutable per tile id, so undo shows the old art by snapshot and the CMO3 export follows the lineage root), an untouched birth quad is re-born over the new art, an edited mesh keeps its vertices and samples the same canvas pixels, a new layer joins under the same file, a removed layer is left alone and reads as Needs review; a CMO3-origin document whose file is not on disk relinks from the layer PNGs the official editor decomposed into it (`cmo3SourceArtOf`).  The watcher (F2): `:reimport`'s `PollingSourceWatcher` stats each listed file through okio (portable to every Kotlin target; no `java.nio`), and `SourceWatchCoordinator` settles a save, hashes the file (SHA-256, `ArtSource.contentHash`; `:format` binary/ContentHash.kt names the digest, `:storage` streams it from a file) so an unchanged save costs nothing, waits for `EditorSession.isQuiescent`, and dispatches `document.reloadArtwork` with a `ReloadScope` under the `import.watchMode` setting (auto / notify / off); a file edited while the document was closed is reported at open, never auto-reloaded.  The matcher (F3): `InventoryLayerMatcher` ranks a lost binding's candidates over the inventory (name, folder path, bounds overlap, size, an exact per-layer content hash) and, when the file was read, pixel similarity, renormalized over the signals present; the inventory keeps a lost layer's row flagged not present while a tile binds it (`ArtSourceLayer.present`), `suggestionsFor` proposes per lost binding (unbound layers only), the reload and the replace run the matcher over the lost bindings before they mint anything and rebind the confident ones in the same step (`ArtworkReloadPlanner.plan(matchThreshold)` → `ReconcileResult.Rebound`, the Match Threshold row on their strips), a proposal a reload left under the bar still names the layer it minted a fresh drawable for and taking it retires that drawable (`ArtworkReload.retiredTiles`, guarded by `tileCarriesNoRigWork`), and the Sources space shows the proposals as review chips.  Still absent: a reader and watcher for Android's SAF uris.  The rest of this section describes the target behavior, now current.
