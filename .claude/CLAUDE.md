...
- **CMO3 — read, and write at the byte/XML level; no edit-to-CMO3 lowering yet.**  A CMO3 is the native format for the Cubism editor.  `Cmo3.write` round-trips an unedited file byte-for-byte, but there is no `PuppetModel → CMO3` synthesis: Save currently re-emits the original bytes untouched.  See `docs/formats/CMO3.md`.
...
- **Semantic validity — CMO3 (v1 primary).** Emit a valid CAFF container whose `main.xml` encodes the same object graph; the official editor's XML deserializer rebuilds it.  Byte-identity is **not** the bar (an edit changes the bytes by construction) — though the current writer does round-trip an *unedited* file byte-for-byte, which is a strong correctness signal, not a contract. **Round-trip is the bar:** open a corpus model, re-save, reopen in the official Cubism Editor, assert no semantic loss (mangled physics or a dropped deformer subtree kills the migration promise).
