...
**Current status: the binding is persisted and editable, the reconcile is not built.**  The identity is ONE representation: the reader's `LayerId.raw` is the key and the reader says how strong it is (`SourceLayer.idIsStable`); it persists on the tile as `AtlasTile.source: SourceLayerRef(sourceId, layerKey, stableKey)`, and the document lists its artwork files with their pixel-free layer inventories as `PuppetModel.sources: List<ArtSource>` (the baseline a reconcile diffs against).  The Sources space shows and edits the table (relink, unbind, Add Artwork).  `:reimport` (`Reconciler`, `LayerMatcher`, `ReconcileResult`, `SourceWatcher`) speaks `SourceLayerRef` and is still contracts only — no reconcile implementation, no platform file-watcher (desktop `WatchService`/Android `FileObserver`), and nothing calls into it from the UI yet.  The rest of this section describes the target behavior to build toward, not current behavior.
...
:reimport     commonMain  — non-destructive reconcile over the model's source bindings
                            (contracts only today). → :format, :runtime (api)
