...
Implemented (live code in `:ui`'s `org.umamo.ui.action`): `Command`/`CommandHandler`/`CommandRegistry` (provided as `LocalCommands`), chords modeled by `KeyChord` + `parseKeyChord` and resolved through a `Keymap` (key position + a logical primary modifier).  `org.umamo.ui.workspace.commands` holds **every** command table — one file per domain (chrome, workspace, document, view, mode, history, object, transform, select, snap, uv, topology, proportional, keyform, file, log), each a builder returning a plain `List<Command>` its owner registers through `registerAll`, which returns the matching cleanup so registration and unregistration cannot drift apart.  A `Command(...)` constructed anywhere else is a bug: the table is the declaration (id, title, availability), and whatever the handler needs arrives as a plain lambda — that is how the app's `file.*` / `logs.export` tables stay in this commonMain package while their pickers and document loader stay up in `jvmAndroidMain`'s `EditorApp`.  Between them they register several dozen commands — mesh editing (grab/scale/rotate/duplicate/merge/rip/connect/vertex-slide, select modes, box/circle select, proportional editing), transform pivot/snap pies, undo/redo, mode toggling, workspace navigation, view toolbar/sidebar toggles, plus the shell-level `palette.toggle`/`area.dragCancel`/`view.fit`/`view.zoom*` and the app's `file.importCmo3`/`file.importMoc3`/`file.exportCmo3`.  **Every "which area does the pointer mean" decision lives in ONE place, `commands/CommandRouting.kt`** (the hovered surface → a camera, a sheet area, a `TransformTarget`, a select-tool area); a command must never re-derive that by comparing `SpaceKind`s itself.  Its input is equally centralized: `Modifier.stampsHoveredSurface` is installed once by `AreaLeaf`, so **every** space kind reports itself — coverage is a property of the area tree, not something a space has to remember, and a new `SpaceKind` is covered the day it is added.  There is exactly ONE resolver: a second, viewport-only one (`PuppetViewportService.activeAreaId`) was retired because the two could disagree.  **A command that needs a viewport and finds none does nothing** — it never reaches back to a viewport the pointer has left (Blender's hovered-area rule).  A request that must execute in one area **carries that area in its payload**, resolved once at dispatch (`SnapRequest`, `SelectLinkedRequest`, `UvSnapRequest`, and the rip / switch-object ids); an overlay collector gates on its own area id and never re-reads a pointer-side volatile at collect time, which could double- or zero-execute across N open viewports.  `handleShellKey` is the live keyboard dispatch (key position → `KeyChord` → `Keymap.commandFor` → `registry.invoke`); the `CommandPalette` lists `registry.all()`.  New operations register a `Command` and bind a chord — never wire a handler into a widget.  The keymap is editable and persisted: `KeymapPersistence` resolves `default`/`cubism`/`blender` presets (settings `input.keybinding.preset`) plus per-command user overrides (`input.keybinding.overrides`), reactively, and a **keybindings editor** (in Settings) supports capture/reassign/conflict-detection/reset per command.
