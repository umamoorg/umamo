...
This refines the thesis: CMO3 read/write stays primary (Cubism interop), but it is a *conversion at the boundary* layered over a custom native format — not the format you work in.  The container is specified in `docs/format/UMA.md` and its codec is built; the app does not Open/Save it yet — see Format Support.
