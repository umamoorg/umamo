...
**Current status: the binding is persisted and editable, and the reconcile by key is built (Phase F1); the watcher and the fuzzy matcher are not.**  The identity is ONE representation: the reader's `LayerId.raw` is the key and the reader says how strong it is (`SourceLayer.idIsStable`); it persists on the tile as `AtlasTile.source: SourceLayerRef(sourceId, layerKey, stableKey)`, and the document lists its artwork files with their pixel-free layer inventories as `PuppetModel.sources: List<ArtSource>` (the baseline a reconcile diffs against).  The Sources space shows and edits the table (relink, unbind, Add Artwork, Reload).  `:reimport`'s `KeyReconciler` classifies bindings by key and `ArtworkReloadPlanner` turns the matched and added halves into an `ArtworkReload` the session applies (`withArtworkReloaded`) and the UI packs and commits (`ReloadArtworkFlow.kt`): a changed layer's tile is superseded by a NEW tile (`<root>~<n>`, `AtlasTile.replaces`; pixels are immutable per tile id, so undo shows the old art by snapshot and the CMO3 export follows the lineage root), an untouched birth quad is re-born over the new art, an edited mesh keeps its vertices and samples the same canvas pixels, a new layer joins under the same file, a removed layer is left alone and reads as Needs review; a CMO3-origin document whose file is not on disk relinks from the layer PNGs the official editor decomposed into it (`cmo3SourceArtOf`).  Still absent: the platform file-watcher (desktop `WatchService`/Android `FileObserver`, F2), `LayerMatcher` and the review chips (F3), and a reader for Android's SAF uris.  The rest of this section describes the target behavior, most of it now current.
...
:reimport     commonMain  — non-destructive reconcile over the model's source bindings: the
                            key reconciler and the reload planner (a changed layer's tile is
                            superseded, never mutated); the watcher and fuzzy matcher are still
                            contracts. → :format, :interop, :runtime (api)
