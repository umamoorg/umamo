...
Implemented (`:ui`'s `org.umamo.ui.action`): `Command`/`CommandHandler`/`CommandRegistry` (as `LocalCommands`), chords via `KeyChord`/`parseKeyChord` resolved through a `Keymap` (key position + a logical primary modifier).  `org.umamo.ui.workspace.commands` holds every command table, one file per domain, each a builder returning `List<Command>` registered through `registerAll` (which returns the matching cleanup, so registration and unregistration can't drift apart).  A `Command(...)` built anywhere else is a bug — the table is the declaration, the handler just a lambda; that split is why the app's `file.*`/`logs.export` tables stay in this commonMain package while their pickers and document loader live in `jvmAndroidMain`'s `EditorApp`.
