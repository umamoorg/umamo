// Shared application-version resolution. Applied via `apply(from = …)` by the app modules that
...
//   umamoVersion        — the full string, e.g. "0.4.0-dev" (jar names, Android versionName)
//   umamoVersionNumeric — the suffix-stripped MAJOR.MINOR.PATCH, e.g. "0.4.0" (jpackage
//                         rejects suffixes, so native installers get the numeric form).  A
//                         released version is already plain (RELEASING.md), so the strip only
//                         changes a dev build from master - whose installer then carries the
//                         NEXT release's number, which is why a dev build's installer is never
//                         handed out.
